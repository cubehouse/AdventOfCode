...
// look for ffmpeg in bin/ and set FFMPEG_PATH to it if it exists
if (fs.existsSync('./bin/ffmpeg.exe')) {
    // set FFMPEG_PATH to the absolute path of ffmpeg
...
export class Window {
    ...
    filter(func) {
        const result = [];
        for (let i = 0; i < this.pixels.length; i++) {
            if (func(this.pixels[i], i % this.width, Math.floor(i / this.width))) {
                result.push(this.pixels[i]);
            }
        }
        return result;
    }
