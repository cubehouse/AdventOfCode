...
export class Screen extends EventEmitter {
    ...
    /**
     * Add a callback function to a given key input
     * @param {string} key 
     * @param {function} func 
     */
    AddInput(key, func) {
        this.screen.key(key, func);
    }
    ...
    FitPositionOntoScreen(x, y, border = 0) {
        ...
        const originalShiftX = this.shiftX;
        const originalShiftY = this.shiftY;
        ...
        // only redraw if we've actually changed the shift
        if (this.shiftX !== originalShiftX || this.shiftY !== originalShiftY) {
            this.Redraw();
        }
    }
