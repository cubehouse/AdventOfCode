...
const obs = new PerformanceObserver((items) => {
    if (!quietMode) {
        const ms = items.getEntries()[0].duration;
        if (ms < 1000) {
...
export class Advent {
    ...
    GetInputRaw(sampleInput = null) {
        if (sampleMode && sampleInput) {
            return Promise.resolve(sampleInput);
        }
    ...
    GetInput(sampleInput = null) {
        return this.GetInputRaw(sampleInput).then((fileBuffer) => {
    ...
    Submit(answer, level = 1) {
        ...
        performance.mark(`Part_${level}`);
        performance.measure(`Part_${level}`, 'A', `Part_${level}`);
    ...
    Assert(query, value, message) {
        ...
        if (sampleMode) {
            ...
            if (query != value) {
                if (message) {
                    console.error(`Assertion failed: Line ${lineNumber}: ${query} != ${value}: ${message}`);
                } else {
                    console.error(`Assertion failed: Line ${lineNumber}: ${query} != ${value}`);
                }
                console.trace();
                //debugger;
            } else {
